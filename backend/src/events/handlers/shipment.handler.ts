...
export function registerShipmentHandlers(): void {
  appEvents.on(APP_EVENTS.SHIPMENT_CREATED, async (payload: { shipmentId: string; email: string }) => {
  ...
  appEvents.on(APP_EVENTS.SHIPMENT_STATUS_UPDATED, (payload: { shipmentId: string; status: string }) => {
